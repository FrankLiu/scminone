...
export NCS_HOME=$(cd "$(dirname "$0")"; pwd)
...
parse_args()
{
	...
	*) usage;;
	...
	case $cur_shell in
		csh|tcsh)
			setup_env_script="${vcssettings_script_name}.csh";;
		*)
			setup_env_script="${vcssettings_script_name}.sh";;
	esac
...
initialize_ncs()
{
	...
	if [ ! -e $workdir/$setup_env_script ]; then
