...
use ncs::core::Component;
package ncs::core::Properties;
@ISA=qw(ncs::core::Component);
...
{
	my $pkg = shift;
	my $obj = $pkg->SUPER::new('ncs::core::Properties',('load','get','set','search'));
    $obj->{'properties_file'} = '';
    $obj->{'properties'} = {};
...
sub load
{
    ...
    if (!&util::Stings::isEmpty($properties_file)){ 
        die("Properties file need to be defined!!");
    }
    ...
    while($line = <PROPFILE>){
		...
		if($is_multiline){
			...
			$v = $v.' '.$line;
			if($line =~ /[^\\]$/){ #end line
				$class->{'default_properties'}->{trim($k)} = trim($v);
    ...
    close(PROPFILE);
	my $import = $class->get("import");
	$import = $class->unescapeProperty("import", $import) if $import;
	if(-e $import){
...
sub merge_with_file
{
	...
	if(-e $file_to_be_merged){
		return $class->merge(core::Properties->new()->load($file_to_be_merged, 1), $override);
	}
	return $class->{'properties'};
}
