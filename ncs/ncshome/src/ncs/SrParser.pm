...
sub srlist
{
	...
	for(my $i=$row_min+1;$i<=$row_max;$i++){ #ignore the head line
		...
		for(my $j=$col_min;$j<=$col_max;$j++){
			my $data = $excel->get_data($sheetname, $i, $j); $data=isEmpty($data)?'':trim($data);
			$line->{$header[$j]} = $data;
			#print "$header[$j] = $data\n"
		}
		push(@srlist, $line);
...
sub srMappings
{
	...
	my $srMappings = {};
	foreach $sr (@srlist){
		my $tcno = $sr->{'#Failed Case No.'}; $tcno = isEmpty($tcno)?'':trim($tcno);
		my $srno = $sr->{'#SR No.'}; $srno = isEmpty($srno)?'':trim($srno);
		#print "$tcno = $srno\n";
